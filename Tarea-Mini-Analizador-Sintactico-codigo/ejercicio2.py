"""
Ejercicio 2 - Mini Analizador Sintactico LR(1)
Gramatica: E -> id + E | id
Con recursion - requiere ciclo iterativo
"""
...
NOMBRE_REGLAS = ["E->id+E", "E->id"]


def analizar_lexico(cadena):
    """Convierte cadena en tokens: id=0, +=1, $=2"""
    tokens = []
    i = 0
    while i < len(cadena):
        c = cadena[i]
...
def analizar_sintactico(cadena):
    """Analiza la cadena usando algoritmo LR(1) con ciclo iterativo."""
    tokens = analizar_lexico(cadena)
    pila = [PESOS, 0]
    pos = 0

    print("Ejercicio 2 - Gramatica: E -> id + E | id")
    ...
    while True:
        ...
        if accion > 0:
            ...
            nombre_regla = NOMBRE_REGLAS[index_regla]

            print(f"r{index_regla + 1} ({nombre_regla})")
...
if __name__ == "__main__":
    for cadena in ["a", "a+b", "a+b+c", "x+y"]:
        print()
        analizar_sintactico(cadena)
        print()
